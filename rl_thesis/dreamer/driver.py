# AUTHOR: Hafner et al. https://github.com/danijar/dreamerv2
...
class Driver:
    def __init__(self, envs, **kwargs):
        print(f"[Driver.init] envs: {envs}")
        self._envs = envs
        self._kwargs = kwargs
        self._on_steps = []
        self._on_resets = []
        self._on_episodes = []
        self._on_driver_done = []
    ...
    def on_driver_done(self, callback):
        self._on_driver_done.append(callback)
    ...
    def __call__(self, policy, steps=0, episodes=0):
        step, episode = 0, 0
        progress_bar_count_steps = steps > episodes
        progress_bar = tqdm(total=max(steps, episodes), leave=False, unit="steps" if progress_bar_count_steps else "episodes")
        while step < steps or episode < episodes:
            ...
            for i, (act, ob) in enumerate(zip(actions, obs)):
                ...
                if ob['is_last']:
                    ...
                    episode += 1
                    if not progress_bar_count_steps:
                        progress_bar.update(1)
            self._obs = obs

        for fn in self._on_driver_done:
            fn()
