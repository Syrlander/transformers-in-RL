...
class GatingMechanism(torch.nn.Module):
    """
    Gating mechanism based on:
    https://github.com/dhruvramani/Transformers-RL/blob/master/layers.py#L39-L58
    """
...
class RelPartialLearnableMultiHeadAttn(RelMultiHeadAttn):
    """
    Relative Multi-Head Attention (RMHA)
    """
    ...
    def forward(self, w, r, r_w_bias, r_r_bias, attn_mask=None, mems=None):
        ...
        Kwargs:
            mems: The tau-step memory tensor. M^(l) \in \mathbb{R}^{\tau x D}
                  (D is the hidden dimension, l being the layer index)
        """
...
class RelPartialLearnableDecoderLayer(nn.Module):
    """
    Modified to include gating layer based on:
    https://github.com/dhruvramani/Transformers-RL/blob/master/layers.py#L182-L215
    """
    
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, gating=True,
                 **kwargs):
    ...
    def forward(self, dec_inp, r, r_w_bias, r_r_bias, dec_attn_mask=None, mems=None):
        output = self.dec_attn(dec_inp, r, r_w_bias, r_r_bias,
                               attn_mask=dec_attn_mask,
                               mems=mems)
        output_tmp = self.gate1(dec_inp, output) if self.gating else output + dec_inp
        output = self.pos_ff(output_tmp)
        output_tmp = self.gate2(output_tmp, output) if self.gating else output_tmp + output
        return output
...
class MemTransformer(nn.Module):
    def __init__(self, d_inp, n_layer, n_head, d_model, d_head, d_inner,
                 ...
                 same_length=False, attn_type=0, clamp_len=-1, gating=True):
    ...
    def _create_params(self):
        # NOTE: Uses torch.zeros rather than torch.Tensor for initialization to avoid potential NaN issues
        if self.attn_type == 0: # default attention
            ...
            self.r_emb = nn.Parameter(torch.zeros(
                    self.n_layer, self.max_klen, self.n_head, self.d_head))
            self.r_w_bias = nn.Parameter(torch.zeros(
                    self.n_layer, self.n_head, self.d_head))
            self.r_bias = nn.Parameter(torch.zeros(
...
if __name__ == '__main__':
    ...
    for i in range(len(data)):
        if i >= (tgt_len+mem_len+ext_len):
            _data = data[i-(tgt_len+mem_len+ext_len):i]
        else:
            _data = data[:i+1]
        pred, _mems = model(_data, *mems)
        if i >= (tgt_len+mem_len+ext_len):
            mems = _mems
            print(pred.shape, len(mems), mems[0].shape)
        else:
            print(pred.shape, mems)
